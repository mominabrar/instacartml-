...
text2vec <- function(n, data, by, with){
  text_data <- cart_products(5,data)
  text_data$product_name <- gsub(",","",text_data$product_name)
  product_user <- aggregate(as.formula(paste(with,by, sep = " ~ ")), data = text_data, paste, collapse = ",")
  text <- as.character(product_user$product_name)
  text <- strsplit(text, ",")
...
text = text2vec(5, orders_train, by = "user_id", with = "product_name")
